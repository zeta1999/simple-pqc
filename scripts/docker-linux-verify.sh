...
if [[ " $sel " == *" 1 "* ]]; then
  ...
  if docker run --rm -v "$ROOT":/src:ro debian:13 bash -euc '
      export DEBIAN_FRONTEND=noninteractive
      apt-get update -qq
      apt-get install -y -qq openssh-server openssh-client openssl golang-go \
      ...
      # host artifacts are read-only mounts; build fresh in the container copy
      ./scripts/run-all.sh
...
if [[ " $sel " == *" 2 "* ]]; then
  ...
  if docker run -d --name pqc-k3s --privileged -p 6443:6443 \
       rancher/k3s:v1.31.5-k3s1 server --disable=traefik,servicelb,metrics-server >/dev/null 2>&1; then
...
if [[ " $sel " == *" 3 "* ]]; then
  ...
  if docker run --rm debian:sid bash -euc '
      ...
      ssh -V
      ssh -Q key-sig | grep -q mldsa44-ed25519 || { echo "no mldsa44-ed25519 in this openssh"; exit 1; }
...
HostKeyAlgorithms ssh-mldsa44-ed25519
PubkeyAcceptedAlgorithms ssh-mldsa44-ed25519
